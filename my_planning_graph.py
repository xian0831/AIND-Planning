...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # check all state node
            for state_node in self.s_levels[level]:
                # if action is in S level
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            # go through list of effnodes
            for state_node in action_node.effnodes:
                if state_node not in next_states:
                    references[state_node] = state_node
                action_node.children.add(references[state_node])
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_rem in node_a2.action.effect_rem:
            if effect_rem in node_a1.action.effect_add:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        for state_node1 in node_a1.parents:
            for state_node2 in node_a2.parents:
                if state_node1.is_mutex(state_node2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_node1 in node_s1.parents:
            for parent_node2 in node_s2.parents:
                if parent_node1 != parent_node2 and parent_node1.is_mutex(parent_node2):
                    results.append(True)
                else:
    ...
    def h_levelsum(self) -> int:
        ...
        def _cost_function(goal):
            for depth, level in enumerate(self.s_levels):
                for state_node in level:
                    # the depth is the admissible cost
                    if state_node.literal == goal:
                        return depth
